...
Так как обучение трёх нейросетей - процесс трудоёмкий, он может занять **десятки минут и более**.
...
**Что это за график?**
Простыми словами: это таблица «кто кого перепутал»: по строкам — истинные классы фруктов, по столбцам — что предсказала модель. Чем темнее клетка, тем больше изображений в неё попало.
**Как это работает?**
Каждая модель прогоняется по отложенной тестовой выборке (20% изображений), и пары «истинный класс → предсказанный класс» подсчитываются. Идеальная модель даёт заполненную диагональ и пустые остальные клетки.
**Ключевые моменты:**
- **LSTM — только диагональ** → ноль ошибок, 100.0% точности
- **MLP — единичные попадания вне диагонали** → 99.9% точности
- **CNN — самые заметные «лишние» клетки** → 99.1% точности: свёрточная сеть чаще остальных путает близкие фрукты
