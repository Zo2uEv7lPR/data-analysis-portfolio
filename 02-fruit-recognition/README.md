...
- **CNN** показывает наиболее заметные off-diagonal элементы: ошибки наблюдаются преимущественно между визуально схожими классами фруктов. Свёрточные фильтры, будучи эффективны для извлечения локальных признаков, одновременно теряют часть глобальной информации из-за max-pooling'а — именно это объясняет чуть более низкую итоговую точность (99.1%) по сравнению с MLP и LSTM.
