...
Скорость перехода оценивается как slope тренда PC1 по году (отрицательные значения — движение в сторону развитого порога). Для прогноза по траектории PC1 каждой страны подбирается четырёхпараметрическая логистическая кривая, после чего решается задача о годе пересечения порога PC1 < 0; горизонт прогноза ограничен 2085 годом.
